...
class Base(Configuration):
    ...
    ALLOWED_HOSTS = config(
        "ALLOWED_HOSTS", default=urlparse(PROXY_BASE_URL).netloc
    ).split(",") + [local_ip]
    ...
    os.environ["DATABASE_URL"] = config(
        "DATABASE_URL",
        default=f"postgresql://hsdbadmin:admin@localhost:5432/hydroserver",
    )
    ...
    ACCOUNT_OWNERSHIP_ENABLED = config(
        "ACCOUNT_OWNERSHIP_ENABLED", default=True, cast=bool
    )
    ...
    EMAIL_CONFIG = dj_email_url.parse(
        config("SMTP_URL", default="smtp://127.0.0.1:1025")
    )
