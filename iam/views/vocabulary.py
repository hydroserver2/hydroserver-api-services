...
@iam_vocabulary_router.get("/user-types", response={200: list[str]}, by_alias=True)
...
@iam_vocabulary_router.get(
    "/organization-types", response={200: list[str]}, by_alias=True
)
def get_organization_types(request):
    ...
    return 200, OrganizationType.objects.filter(public=True).values_list(
        "name", flat=True
    )
