        ...
        if user is None:
            ...
            return self.filter(
                ...
                Q(workspace__collaborators__user=user,
                  workspace__collaborators__role__permissions__resource_type__in=["*", "ResultQualifier"],
...
class ResultQualifier(models.Model, PermissionChecker):
    ...
    def can_user_create(cls, user: Optional["User"], workspace: "Workspace"):
        return cls.check_create_permissions(user=user, workspace=workspace, resource_type="ResultQualifier")
