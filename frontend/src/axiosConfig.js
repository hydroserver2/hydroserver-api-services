...
axios.defaults.baseURL = `${import.meta.env.MODE === 'development' ? 'http://127.0.0.1:8000' : PROXY_BASE_URL}/api/`
...
axios.interceptors.request.use(
  (config) => {
    ...
    if (refresh_token) config.headers.Refresh_Authorization = `Bearer ${refresh_token}`
    return config;
  },
