...
from etl.schemas import EtlSystemPlatformPostBody, EtlSystemPlatformPatchBody, EtlSystemPlatformGetResponse
...
def test_list_etl_system_platform(django_assert_num_queries, get_user, user, workspace, length, max_queries):
...
@pytest.mark.parametrize("user, etl_system_platform, message, error_code", [
    ("owner", "ff0f462f-7456-40d7-93b6-77806176d08f", "Global ETL System Platform", None),
    ("owner", "b3d13d31-95a1-482e-a35b-ae24e78d519e", "Workspace ETL System Platform", None),
    ("admin", "ff0f462f-7456-40d7-93b6-77806176d08f", "Global ETL System Platform", None),
    ("admin", "b3d13d31-95a1-482e-a35b-ae24e78d519e", "Workspace ETL System Platform", None),
    ("editor", "ff0f462f-7456-40d7-93b6-77806176d08f", "Global ETL System Platform", None),
    ("editor", "b3d13d31-95a1-482e-a35b-ae24e78d519e", "Workspace ETL System Platform", None),
    ("viewer", "ff0f462f-7456-40d7-93b6-77806176d08f", "Global ETL System Platform", None),
    ("viewer", "b3d13d31-95a1-482e-a35b-ae24e78d519e", "Workspace ETL System Platform", None),
    ("anonymous", "ff0f462f-7456-40d7-93b6-77806176d08f", "Global ETL System Platform", None),
...
def test_get_etl_system_platform(get_user, user, etl_system_platform, message, error_code):
    if error_code:
...
def test_create_etl_system_platform(get_user, user, workspace, message, error_code):
    etl_system_platform_data = EtlSystemPlatformPostBody(
        name="New", interval_schedule_supported=True, crontab_schedule_supported=True, workspace_id=uuid.UUID(workspace)
    )
    if error_code:
        with pytest.raises(HttpError) as exc_info:
        ...
        assert (etl_system_platform_create.interval_schedule_supported ==
                etl_system_platform_data.interval_schedule_supported)
        assert (etl_system_platform_create.crontab_schedule_supported ==
...
def test_edit_etl_system_platform(get_user, user, etl_system_platform, message, error_code):
    ...
    if error_code:
        with pytest.raises(HttpError) as exc_info:
            etl_system_platform_service.update(
                user=get_user(user), uid=uuid.UUID(etl_system_platform), data=etl_system_platform_data
            )
        assert exc_info.value.status_code == error_code
        assert exc_info.value.message.startswith(message)
    else:
        ...
        assert (etl_system_platform_update.interval_schedule_supported ==
                etl_system_platform_data.interval_schedule_supported)
        assert (etl_system_platform_update.crontab_schedule_supported ==
                etl_system_platform_data.crontab_schedule_supported)
...
@pytest.mark.parametrize("user, etl_system_platform, message, error_code", [
    ...
    ("admin", "ff0f462f-7456-40d7-93b6-77806176d08f", "ETL system platform has one or more active instances", 409),
...
def test_delete_etl_system_platform(get_user, user, etl_system_platform, message, error_code):
    if error_code:
        with pytest.raises(HttpError) as exc_info:
