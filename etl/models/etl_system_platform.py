        ...
        if user is None:
            ...
            return self.filter(
                ...
                Q(workspace__collaborators__user=user,
                  workspace__collaborators__role__permissions__resource_type__in=["*", "EtlSystemPlatform"],
...
class EtlSystemPlatform(models.Model, PermissionChecker):
    ...
    workspace = models.ForeignKey(Workspace, related_name="etl_system_platforms", on_delete=models.DO_NOTHING,
    ...
    def can_user_create(cls, user: Optional["User"], workspace: "Workspace"):
        return cls.check_create_permissions(user=user, workspace=workspace, resource_type="EtlSystemPlatform")
    ...
    def get_user_permissions(self, user: Optional["User"]) -> list[Literal["edit", "delete", "view"]]:
        user_permissions = self.check_object_permissions(user=user, workspace=self.workspace,
                                                         resource_type="EtlSystemPlatform")
    ...
    def delete_contents(filter_arg: models.Model, filter_suffix: Optional[str]):
        ...
        etl_system_platform_relation_filter = f"etl_system_platform__{filter_suffix}" \
            if filter_suffix else "etl_system_platform"

        EtlSystem.delete_contents(filter_arg=filter_arg, filter_suffix=etl_system_platform_relation_filter)
        EtlSystem.objects.filter(**{etl_system_platform_relation_filter: filter_arg}).delete()
        EtlConfiguration.objects.filter(**{etl_system_platform_relation_filter: filter_arg}).delete()
