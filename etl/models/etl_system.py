...
class EtlSystem(models.Model, PermissionChecker):
    ...
    etl_system_platform = models.ForeignKey(
        EtlSystemPlatform, related_name="etl_systems", on_delete=models.DO_NOTHING
    )
    ...
    def can_user_create(cls, user: Optional["User"], workspace: "Workspace"):
        return cls.check_create_permissions(
            user=user, workspace=workspace, resource_type="EtlSystem"
        )
    ...
    def get_user_permissions(
        ...
        user_permissions = self.check_object_permissions(
            user=user, workspace=self.workspace, resource_type="EtlSystem"
        )
    ...
    def delete_contents(filter_arg: models.Model, filter_suffix: Optional[str]):
        ...
        etl_system_relation_filter = (
            f"etl_system__{filter_suffix}" if filter_suffix else "etl_system"
        )

        DataSource.delete_contents(
            filter_arg=filter_arg, filter_suffix=etl_system_relation_filter
        )
