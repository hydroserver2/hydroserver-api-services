...
class EtlConfigurationService(ServiceUtils):
    @staticmethod
    def get_etl_configuration_for_action(user: User, uid: uuid.UUID, action: Literal["view", "edit", "delete"],
                                         etl_system_platform_id: Optional[uuid.UUID] = None, raise_400: bool = False):
        if etl_system_platform_id and not EtlSystemPlatform.objects.filter(pk=etl_system_platform_id).exists():
            raise HttpError(404 if not raise_400 else 400, "ETL system platform does not exist")

        try:
            etl_configuration = EtlConfiguration.objects.select_related("etl_system_platform__workspace").get(pk=uid)
        except EtlConfiguration.DoesNotExist:
            raise HttpError(404 if not raise_400 else 400, "ETL configuration does not exist")

        etl_configuration_permissions = etl_configuration.get_user_permissions(user=user)

        if "view" not in etl_configuration_permissions:
            raise HttpError(404 if not raise_400 else 400, "ETL configuration does not exist")

        if action not in etl_configuration_permissions:
            raise HttpError(403 if not raise_400 else 400,
    ...
    @staticmethod
    def list(user: Optional[User], workspace_id: Optional[uuid.UUID], etl_system_platform_id: Optional[uuid.UUID]):
    ...
    def get(self, user: Optional[User], uid: uuid.UUID, etl_system_platform_id: uuid.UUID):
        return self.get_etl_configuration_for_action(user=user, uid=uid,
                                                     etl_system_platform_id=etl_system_platform_id, action="view")

    def create(self, user: User, etl_system_platform_id: uuid.UUID, data: EtlConfigurationPostBody):
        etl_system_platform = etl_system_platform_service.get_etl_system_platform_for_action(
        ...
        if not etl_system_platform.workspace or not EtlConfiguration.can_user_create(
            ...
            raise HttpError(403, "You do not have permission to create this ETL configuration")
    ...
    def update(self, user: User, etl_system_platform_id: uuid.UUID, uid: uuid.UUID, data: EtlConfigurationPatchBody):
        etl_configuration = self.get_etl_configuration_for_action(user=user, uid=uid,
                                                                  etl_system_platform_id=etl_system_platform_id,
                                                                  action="edit")
        etl_configuration_data = data.dict(include=set(EtlConfigurationFields.model_fields.keys()), exclude_unset=True)
