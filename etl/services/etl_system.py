...
class EtlSystemService(ServiceUtils):
    @staticmethod
    def get_etl_system_for_action(user: User, uid: uuid.UUID, action: Literal["view", "edit", "delete"],
        ...
        if action not in etl_system_permissions:
            raise HttpError(403 if not raise_400 else 400, f"You do not have permission to {action} this ETL system")

        return etl_system

    @staticmethod
    def list(user: Optional[User], workspace_id: Optional[uuid.UUID], etl_system_platform_id: uuid.UUID):
    ...
    def create(self, user: User, data: EtlSystemPostBody):
        ...
        etl_system_platform = self.handle_http_404_error(etl_system_platform_service.get, user=user,
                                                         uid=data.etl_system_platform_id)

        if etl_system_platform.workspace and etl_system_platform.workspace != workspace:
            if not workspace:
                workspace = etl_system_platform.workspace
            else:
                raise HttpError(400, "ETL systems must share a workspace with the ETL system platform")
    ...
    def update(self, user: User, uid: uuid.UUID, data: EtlSystemPatchBody):
        ...
        etl_system_data = data.dict(include=set(EtlSystemFields.model_fields.keys()), exclude_unset=True)
