...
class EtlSystemPlatformService(ServiceUtils):
    @staticmethod
    def get_etl_system_platform_for_action(user: User, uid: uuid.UUID, action: Literal["view", "edit", "delete"]):
        try:
            etl_system_platform = EtlSystemPlatform.objects.select_related("workspace").get(pk=uid)
        except EtlSystemPlatform.DoesNotExist:
            raise HttpError(404, "ETL system platform does not exist")

        etl_system_platform_permissions = etl_system_platform.get_user_permissions(user=user)

        if "view" not in etl_system_platform_permissions:
            raise HttpError(404, "ETL system platform does not exist")

        if action not in etl_system_platform_permissions:
            raise HttpError(403, f"You do not have permission to {action} this ETL system platform")
    ...
    def get(self, user: Optional[User], uid: uuid.UUID):
        return self.get_etl_system_platform_for_action(user=user, uid=uid, action="view")
    ...
    def create(self, user: User, data: EtlSystemPlatformPostBody):
        ...
        if not EtlSystemPlatform.can_user_create(user=user, workspace=workspace):
            raise HttpError(403, "You do not have permission to create this ETL system platform")
    ...
    def update(self, user: User, uid: uuid.UUID, data: EtlSystemPlatformPatchBody):
        etl_system_platform = self.get_etl_system_platform_for_action(user=user, uid=uid, action="edit")
        etl_system_platform_data = data.dict(include=set(EtlSystemPlatformFields.model_fields.keys()),
                                             exclude_unset=True)
    ...
    def delete(self, user: User, uid: uuid.UUID):
        etl_system_platform = self.get_etl_system_platform_for_action(user=user, uid=uid, action="delete")
