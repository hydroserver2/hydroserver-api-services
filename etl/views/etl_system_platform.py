...
@transaction.atomic
def create_etl_system_platform(
    request: HydroServerHttpRequest, data: EtlSystemPlatformPostBody
):
    """
...
def get_etl_system_platform(
    request: HydroServerHttpRequest, etl_system_platform_id: Path[uuid.UUID]
):
    """
    ...
    return 200, etl_system_platform_service.get(
        user=request.authenticated_user, uid=etl_system_platform_id
    )
...
@transaction.atomic
def update_etl_system_platform(
    ...
    return 200, etl_system_platform_service.update(
        user=request.authenticated_user, uid=etl_system_platform_id, data=data
    )
...
@transaction.atomic
def delete_etl_system_platform(
    request: HydroServerHttpRequest, etl_system_platform_id: Path[uuid.UUID]
):
    """
